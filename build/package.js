...
var header = "// " + new Date(newestFileTime).toISOString() + "\n// " + packageJson.name + " v" + packageJson.version;

var concatenatedFile = result.join("\n")

fs.outputFileSync(path.join(libraryDir, 'target', 'define-single', libraryName + ".js"),
		header + " in a single file (needs a global 'define' function).\n" + concatenatedFile
...
fs.outputFileSync(path.join(libraryDir, 'target', 'single', libraryName + ".js"), [
	header + " in a self-contained file, suitable for the browser.\n",
	fs.readFileSync(path.join(__dirname, "definitionFunction.js"), {encoding: 'utf8'}) + "('" + libraryName +"', function() {",
	"\t" + moduleSystemContent.replace(/\n/g, '\n\t'),
	concatenatedFile.replace(/\n/g, '\n\t'),
	"\treturn require('./" + fullPathToModuleName(packageJson.main) + "');",
	"});"
].join("\n"));